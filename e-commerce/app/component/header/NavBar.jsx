...
const NavBar = () => {
  ...
  // Define authentication-related paths where navbar shouldn't appear
  const authRoutes = [
    '/sign-in',
    '/sign-up',
    '/sign-in/[[...index]]',
  ...
  return (
    <header className="md:mx-10 xl:mx-10 fixed top-0 left-0 right-0 z-50 backdrop-blur-md bg-black/80 border-b border-gray-800">
      ...
      {/* Mobile Menu - Show only login and register buttons */}
      {isMobileMenuOpen && (
        <div className="absolute top-16 left-0 right-0 bg-black/95 border-t border-gray-800 md:hidden">
          <div className="max-w-7xl mx-auto px-4 py-6 flex flex-col space-y-4">
            <SignedOut>
              <SignInButton>
                <button className="w-full py-3 px-4 text-center text-[#E7F0DC] 
                  hover:bg-[#597445]/20 rounded-lg transition-colors"
                >
                  Login
                </button>
              </SignInButton>
              <SignUpButton>
                <button className="w-full py-3 px-4 text-center bg-[#597445] 
                  text-[#E7F0DC] rounded-lg hover:bg-[#597445]/80 
